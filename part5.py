...
# DATA LOADING FUNCTIONS (For all pages)
...
def get_top_tracks_for_artist(selected_artist_name, db_path):
    ...
    #  WRANGLING Outliers and Invalid Records 
...
def home_page():
    ...
    with right:
        ...
        ax2.barh(top10["name"], top10["followers"], color="#3D6E12")
    ...
    fig = px.scatter(
        ...
        hover_name="name",
        opacity=0.4,
        color_discrete_sequence=["#3D6E12"])
    ...
    with left:     
        ...
        ax4.barh(top_genres.index, top_genres.values, color="#3D6E12")
    ...
    with right: 
        ...
        df.boxplot(column="artist_popularity", by="genre_count", ax=ax5,
            boxprops=dict(linewidth=2, color="#3D6E12"),
            whiskerprops=dict(linewidth=2, color="#3D6E12"),
            capprops=dict(linewidth=2, color="#3D6E12"),
            medianprops=dict(linewidth=2, color="#3D6E12"))
